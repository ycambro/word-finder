...
*/
nodito* crear_nodo(char texto[3024]){
    nodito* nn = calloc(1, sizeof(nodito));
	sprintf(nn -> nombre, "%s", texto);
	//nn -> nombre = texto;
	return nn;
}
...
noditoc* crear_nodo_c(char* termino, listilla* archivo){
	...
	int llaveTamanno = strlen(termino);
	nn->valor = funcion_hash(termino, llaveTamanno);
...
int insertar_al_final(listilla* lista, char* annadir) {
	...
	if (act == NULL) { 					  //Si la lista esta vacia
		...
		while(act -> sigt != NULL) {
			if (strcmp(act -> nombre, annadir) == 0) {
				return -1;
			} else {
...
int imprimir_lista(listilla* lista) {
	...
	if (act == NULL) { 							//Si la lista esta vacia pues...
		...
		printf("%s - ", act -> nombre); 	//Imprime el valor en la lista
...
void imprimirPreorden(noditoc* nodoActual){
	if(nodoActual != NULL){
		printf("%s - ",nodoActual->terminos);
		imprimirPreorden(nodoActual->izq);
		imprimirPreorden(nodoActual->der);
	}
}
...
noditoc* padre(noditoc* raiz, noditoc* nodo){
	if (raiz == NULL || raiz == nodo){
		return NULL;
	} else if (raiz->izq == nodo || raiz->der == nodo) {
		return raiz;
...
void reacomodar(noditoc* nodo, arbolitoc* arbol){
	noditoc* papa = padre(arbol->raiz, nodo);
	noditoc* abuelo = padre(arbol->raiz, papa);

	if (papa == arbol -> raiz && papa -> izq == nodo){ // Caso Zig
		rotar_der(papa, nodo);
		//arbol -> raiz == nodo;
	} else if (papa == arbol -> raiz && papa->der == nodo){ // Caso Zag
		rotar_izq(papa, nodo);
		//arbol -> raiz == nodo;
	} else if (abuelo == arbol -> raiz && abuelo->der == papa && papa->izq == nodo){ // Caso Zig-Zag
		rotar_der(papa, nodo);
		rotar_izq(abuelo, nodo);
		arbol -> raiz = nodo; // Se actualiza la raiz
	} else if (abuelo == arbol -> raiz && abuelo -> izq == papa && papa -> der == nodo){ // Caso Zag-Zig
		rotar_izq(papa, nodo);
		rotar_der(abuelo, nodo);
		arbol -> raiz = nodo; // Se actualiza la raiz
	} else if (abuelo == arbol -> raiz && abuelo -> der == papa && papa -> der == nodo) { // Caso Zag-Zag
		rotar_izq(abuelo, papa);
		rotar_izq(papa, nodo);
		arbol -> raiz = nodo; // Se actualiza la raiz
	} else if (abuelo == arbol -> raiz && abuelo -> izq == papa && papa -> izq == nodo) { // Caso Zig-Zig
		rotar_der(abuelo, papa);
		rotar_der(papa, nodo);
		arbol -> raiz = nodo; // Se actualiza la raiz
	} else if (abuelo->der == papa && papa->izq == nodo){ // Caso Zig-Zag
		rotar_der(papa, nodo);
		rotar_izq(abuelo, nodo);
	} else if (abuelo -> izq == papa && papa -> der == nodo){ // Caso Zag-Zig
		rotar_izq(papa, nodo);
		rotar_der(abuelo, nodo);
	} else if (abuelo -> der == papa && papa -> der == nodo) { // Caso Zag-Zag
		rotar_izq(abuelo, papa);
		rotar_izq(papa, nodo);
	} else if (abuelo -> izq == papa && papa -> izq == nodo) { // Caso Zig-Zig
		rotar_der(abuelo, papa);
		rotar_der(papa, nodo);
	}
}
...
void insertar_chaflaneado(arbolitoc* arbol, char* termino, listilla* lista){
	...
	if(actual == NULL){
		...
		insertar_chaflaneado_auxiliar(arbol -> raiz, nn);
		while(arbol -> raiz != nn){
			reacomodar(nn, arbol);
...
noditoc* buscar_nodo_aux(noditoc* nodo, int valor, char* palabra){
	if(nodo == NULL){ // Si no se encuentra la palabra
		return NULL;
	}
	if(nodo->valor == valor && strcmp(nodo->terminos, palabra)){
		return nodo;
	}
	if(nodo->valor >= valor){
		return buscar_nodo_aux(nodo->izq, valor, palabra);
	}
	if(nodo->valor < valor){
		return buscar_nodo_aux(nodo->der, valor, palabra);
	}
}
...
noditoc* buscar_nodo_c(arbolitoc* arbol, char* termino){
	int llaveTammanno = strlen(termino);
	int valor = funcion_hash(termino, llaveTammanno);

	if(arbol->raiz == NULL){ // Si el arbol esta vacio
		return NULL;
	}
	noditoc* nodo_encontrado = buscar_nodo_aux(arbol->raiz, valor, termino);
	while(arbol -> raiz != nodo_encontrado){
		reacomodar(nodo_encontrado, arbol);
	}
